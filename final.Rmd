...
t_strategies <- c("Prioritize buying SMGs, occasionally buy armor and helmets", "Highly prioritize shotguns over anything else", "Camp with rifles and lots of armor and helmet", "Prioritize rifles over other weapons", "Prioritize snipers over other weapons", "N/A", "Rush other side with grenades, pistols, and lots of armor", "Camp with snipers", "Prioritize pistols over anything else")
names(t_strategies) <- c(1, 2, 3, 4, 5, 6, 7, 8, 9)

View(ct_kmeans$centers)
ct_strategies <- c("Buy the defusing kit, armor, and rifles", "Rush with shotguns")
names(t_strategies) <- c(1, 2)
```
...
#t_winners$t_kmeans.cluster <- factor(t_winners$t_kmeans.cluster, labels=t_strategies)
head(t_winners)

ct_winners <- data.frame(ct_winners, ct_kmeans$cluster)
#ct_winners$ct_kmeans.cluster <- factor(ct_winners$ct_kmeans.cluster, labels=ct_strategies)
head(ct_winners)
```
...
col_order <- append(colnames(csgo_round), c("t_kmeans.cluster", "ct_kmeans.cluster"))
